...
Сборка бандла отдельно: `./scripts/build-app.sh` (результат в `dist/`). Подпись ad-hoc, поэтому macOS может попросить заново выдать Accessibility после пересборки; если в Keychain есть сертификат с именем `ns2controller`, скрипт подпишет им.
