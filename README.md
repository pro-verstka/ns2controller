...
- Режим `mouse` понимает `recenter` (по умолчанию `true`): пока игра прячет курсор (режим камеры), он тихо возвращается в центр окна у краёв, чтобы клики всегда попадали в игру; в меню с видимым курсором возврата нет, только ограничение окном. Для игр, где курсор всегда видим (Disco Elysium), можно поставить `false`.
- Готовые профили лежат в `profiles/` репозитория, скопируй нужный в папку профилей или создай профиль в приложении и вставь содержимое:
...
По Bluetooth контроллер не является HID-устройством для macOS, поэтому Steam, CrossOver и игры на GameController framework его не увидят без виртуального контроллера. Зато мост работает без кабеля: приложение подключается к контроллеру через CoreBluetooth и получает тот же поток кнопок и стиков.

- В приложении: вкладка «Контроллер» → переключатель «Bluetooth». macOS попросит разрешение на Bluetooth. Отсоедини кабель, зажми кнопку синхронизации на контроллере, пока индикаторы не побегут; статус сменится на «подключён». Дальше мост запускается как обычно.
- В терминале: `ns2ctl ble` показывает статус, кнопки и стики, `ns2ctl kbm --ble` запускает мост по Bluetooth (разрешение Bluetooth нужно Terminal).
- Сопряжение без бондинга: контроллер не появляется в системных настройках Bluetooth, это нормально. При потере связи приложение ищет его снова, достаточно нажать любую кнопку или снова зажать синхронизацию.
- Калибровка стиков по Bluetooth недоступна, берётся последняя снятая по USB (`~/Library/Application Support/ns2controller/calibration.json`); один раз подключи контроллер кабелем, чтобы она появилась.
- Протокол: характеристика `7492866c-ec3e-4619-8258-32755ffcc0f9` без инициализации стримит 11-байтный отчёт того же формата, что USB report 9 без байта ID (2 байта счётчика, 3 байта кнопок, 6 байт стиков). Если её нет, приложение подписывается на все notify-характеристики и берёт первую с отчётами длиной от 11 байт. LED и вибрация по Bluetooth пока не поддержаны.
