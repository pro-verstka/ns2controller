...
Два варианта: приложение с интерфейсом или CLI-демон. Обычно нужен первый.

### Приложение NS2 Controller

```bash
./scripts/install-app.sh
```

Собирает `NS2 Controller.app` (SwiftUI, иконка в строке меню, ad-hoc подпись), кладёт в `/Applications`, выключает CLI-LaunchAgent, если он был установлен, и запускает приложение. Что умеет:

- **Контроллер**: статус USB/HID, автопробуждение при подключении, кнопки «Разбудить»/«Сбросить», LED игрока, заводская калибровка стиков и живая схема кнопок и стиков.
- **Мост**: запуск/остановка клавиатурно-мышиного моста, выбор и создание профилей, визуальный редактор (режимы стиков, слайдеры, назначение клавиш кнопкой «Записать» или выбором кнопок мыши), пауза. Для инъекции приложению нужно разрешение Accessibility: при первом запуске моста появится системный запрос.
- **Интеграции**: установка маппинга в Steam (закрывает и перезапускает Steam, бэкап `config.vdf` рядом) и в бутылку CrossOver.
- **Журнал**: лог приложения (`~/Library/Logs/ns2controller-app.log`), дескриптор устройства и сырые отчёты.
- **Настройки**: запуск при входе в систему, автопробуждение, LED, отключение параллельного LaunchAgent.

Сборка бандла отдельно: `./scripts/build-app.sh` (результат в `dist/`). Перед первой сборкой стоит один раз выполнить `./scripts/make-signing-identity.sh`: он создаёт в связке ключей самоподписанный сертификат `ns2controller`, и приложение подписывается им. Без него подпись ad-hoc, а такой подписи macOS доверяет только по хэшу бинарника: после каждой пересборки переключатель Accessibility выглядит включённым, но не действует, и его нужно снимать и ставить заново (или `tccutil reset Accessibility com.p1rate.ns2controller.app`).

### CLI-демон без интерфейса

```bash
./scripts/install.sh
```

Собирает release-бинарник, кладёт его в `~/.local/bin/ns2ctl`, ставит LaunchAgent `com.p1rate.ns2controller` (автозапуск при логине, `KeepAlive`) и пишет лог в `~/Library/Logs/ns2controller.log`. Вместе с приложением не нужен: оба будут будить контроллер.

Удаление демона: `./scripts/uninstall.sh`. `ns2ctl` полезен и рядом с приложением: диагностика, `kbm` из Terminal, `send`/`flash`.
...
- Готовые профили лежат в `profiles/` репозитория: `lies-of-p.json` повторяет стандартную раскладку Lies of P (Xbox-позиции кнопок → клавиатурные дефолты игры: Nintendo A = уклонение/бег `space`, B = взаимодействие `e`, X = Fable Arts `f`, Y = предмет `r`, R/ZR = атаки ЛКМ/ПКМ, L = блок `shift`, ZL = Legion Arm `ctrl`, RS = лок-он, D-pad = слоты пояса `t`/`g`, смена Legion Arm `x` и оружия `tab`, + = меню, − = записи, GL/GR дублируют уклонение и блок). Скопируй файл в папку профилей или создай профиль в приложении и вставь содержимое.
